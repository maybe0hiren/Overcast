...
5. **addFile(filePath, fileName, fileFormat, uniqueID=None)**: Adds new row and returns the UID. Also, generates the UID if not passed [[stringPlay.py]]
6. **deleteFile(uniqueID)**: Moves the file into "Trash"
7. **makeLink(uniqueID)**: Retrieves the "link" column of the row
8. **editPath(uniqueID, newPath, newName)**: Changes the path of the file and updates the UID [[stringPlay.py]]
